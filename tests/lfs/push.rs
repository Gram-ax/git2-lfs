...
#[rstest]
fn lfs_find_objects_to_push(_sandbox: TempDir, #[with(&_sandbox)] repo: git2::Repository) -> Result<(), anyhow::Error> {
...
#[rstest]
fn lfs_find_objects_to_push_multiple_commits(_sandbox: TempDir, #[with(&_sandbox)] repo: git2::Repository) -> Result<(), anyhow::Error> {
	let workdir = repo.workdir().unwrap();
	let sig = repo.signature()?;
